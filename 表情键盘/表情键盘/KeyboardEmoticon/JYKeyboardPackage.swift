...
extension JYKeyboardPackage {
    ...
    class func createAttributedString(text: String, font: UIFont) -> NSMutableAttributedString {
        ...
        let results = regex.matchesInString(text, options: .ReportProgress, range: NSRange(location: 0, length: text.characters.count))
    ...
    class func findEmoticonPngPath(emoticonChs: String) -> String? {
        ...
        for packege in packeges {
            ...
            for emoticon in emoticons {
                ...
                // 2.2 如果表情字符串存在，进行匹配。
                if emoticonChs == chs {
                    return emoticon.pngPath // 匹配到了，就不需要再继续遍历下一个表情包，直接退出循环
                }
            }
        }
